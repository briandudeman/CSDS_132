...
public class DrawingPad {
     ...
     * if it is null it actually calls canvas.getGraphics, if it is not null, the field is just returned
     ...
     * @param shape the shape to be drawn. one of the shape classes we made. must have a getLines() method
     */
    public void draw(Polygon shape) {
        // for each line, draw it 
        for (Line line : shape.getLines()) {
            this.getGraphicsContext().drawLine((int)line.getFirstPoint().getX(), (int)line.getFirstPoint().getY(), (int)line.getSecondPoint().getX(), (int)line.getSecondPoint().getY());
        }
    }
