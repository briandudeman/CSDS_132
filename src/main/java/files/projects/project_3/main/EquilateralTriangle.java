...
public class EquilateralTriangle extends Triangle implements NGon {
    ...
    /**
    * the constructor which passes the points to the super class which is Triangle
    and sets the center and
    * side lengths
    ...
    */
    public EquilateralTriangle(Point center, double sideLength) {
        super(new Point[] {
        new Point(center.getX() - .5 * sideLength, center.getY() - 3 / (2 *
        Math.tan(Math.PI / 3))),
        new Point(center.getX() + .5 * sideLength, center.getY() - 3 / (2 *
        Math.tan(Math.PI / 3))),
        new Point(center.getX(), center.getY() +
        (Math.sqrt((Math.pow(sideLength, 2) - (Math.pow(sideLength, 2)) / 4)) - 3 / (2 *
        Math.tan(Math.PI / 3))))
        });
        this.center = center;
